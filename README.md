...
- Copy a file/directory's absolute path to the clipboard (`y`, or `Y` for the pane's own directory), and duplicate the entry under the cursor within the same directory (`c`)
...
| `y` | Copy the absolute path of the entry under the cursor to the system clipboard (described below). On the `..` row it copies the parent directory — the one `..` would take you to |
| `Y` (Shift+y) | Copy the active pane's own directory path to the clipboard, wherever the cursor happens to be |
...
**How `y`/`Y` (copy path) work:** Without pulling in any extra dependency crates, they write to the clipboard using a terminal escape sequence called [OSC 52](https://sw.kovidgoyal.net/kitty/clipboard/#clipboard-escape-code). Its advantage is that it works even over SSH or from inside tmux (if tmux is configured with the equivalent of `set-clipboard on`). On unsupported terminals, the escape sequence is simply ignored — no error, no crash (since there's no reliable way to detect support in advance, pressing `y` always logs `copied: /path/to/file`). Inside a Virtual Directory both copy the archive-internal form instead (`archive.zip:/inner/path`), the same way the pane header shows it.
...
Press `G` (Shift+g) on a marked row to see the change behind the marker — `git diff HEAD` for that entry, in the built-in viewer (see the key table above for the details). The marker itself is read from the last probe rather than by asking `git` again, so this action is exactly as fresh as the column is: `Ctrl+R` re-probes, and with `show_git_status = false` there are no markers and therefore no `G` either.
