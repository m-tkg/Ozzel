...
- [ ] エントリ行から左ボタンでドラッグすると、通過した行のマーク状態がトグルされる（未マークの行がマークされる）ことを確認する
- [ ] マーク済みの範囲を再度ドラッグすると、まとめて解除されることを確認する（ドラッグでの選択解除）
- [ ] ドラッグ中に同じ行を何度も行き来しても、その行のマーク状態が意図せず反転を繰り返さないことを確認する（最初にその行を通過した時点の1回だけトグルされる）
...
- [ ] ビューア表示中にホイールを回すとスクロールすることを確認する（ログビューア・ヘルプ画面でも同様）
