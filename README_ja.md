...
![実際の操作を録画した GIF。カーソルを動かし、`Enter` で `src/` に入り、`Space` で 3 ファイルをマークして `C` で反対側のペインへコピー。確認ダイアログに答えると、結果がログペインに表示される](docs/images/demo-tour.gif)
...
一覧を渡り歩く 3 つの方法を順に実行したところです。`/` でマッチするものだけに絞り込み、`Esc` で解除、`\`（次に説明するプレフィックスジャンプ）は一覧をそのままにカーソルだけを動かし、`F`（コマンドパレット）はアクション（ここでは `toggle_hidden`）を名前で実行します。

![`/re` と入力して一覧を 2 件に絞り込み、Esc で解除。`\sc` でカーソルを `scripts` へジャンプさせ、続いて `F` のコマンドパレットを開いて `toggle_hidden` に絞り込んで実行し、隠しファイルが現れる様子](docs/images/demo-search.gif)
...
![`release-1.4.0.zip` の上で Enter を押して中に入り、`release/share/doc` まで降りていく様子（ペインヘッダにはアーカイブ内のパスが表示される）。2 つのファイルを `Space` でマークし、それだけを反対側のペインへコピーすると、その 2 つだけが展開される](docs/images/demo-virtual-directory.gif)
...
![`S` で設定画面を開き、カテゴリ → Colors → directory と辿って色一覧から `magenta` を選択。`Esc` で戻ると、ディレクトリの行がすべて新しい色で描き直されており、ログには `config reloaded` が表示されている](docs/images/demo-settings.gif)
